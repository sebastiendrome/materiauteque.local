...
function removeDirs(imgDirs){
	$.ajax({
		...
		url: rel+'c/php/admin/admin_ajax.php?removeDirs='+imgDirs,
		type: 'GET',
...
function updateTable(table, id, col, value){
	$.ajax({
		...
		url: rel+'c/php/admin/admin_ajax.php?updateTable&table='+table+'&id='+id+'&col='+col+'&value='+value,
		type: 'GET',
		// on success show message
		success : function(msg) {
...
function deleteItem(table, item_id){
	$.ajax({
		url: rel+'c/php/admin/admin_ajax.php?deleteItem&table='+table+'&id='+item_id,
		type: 'GET',
		success : function(msg){
			//return msg;
			var pre = msg.substr(0,2);
...
function create_article(fields){
	...
	$.ajax({
		// Server script to process the ajax call 
		url: rel+'c/php/admin/admin_ajax.php?create_article'+query_string,
		type: 'GET',
...
function duplicate_vrac_article(id, old_poids, old_prix){
	$.ajax({
		// Server script to process the ajax call
		url: rel+'c/php/admin/admin_ajax.php?vrac_vente&original_id='+id+'&old_poids='+old_poids+'&old_prix='+old_prix,
		type: 'GET',
...
function updatePaniersModal(){
	$.ajax({
		url: rel+'c/php/admin/admin_ajax.php?updatePaniersModal',
		type: 'GET',
		success : function(msg){
...
function display_panier(panierId, context){
	...
	$.ajax({
		// Server script
		url: rel+'c/php/admin/admin_ajax.php?display_panier&id='+panierId+'&context='+context,
		type: 'GET',
...
function display_article_panier(articleId, panierId, context){
	...
	$.ajax({
		...
		url: rel+'c/php/admin/admin_ajax.php?display_article_panier&articleId='+articleId+'&panierId='+panierId+'&context='+context,
		type: 'GET',
...
function create_panier(article_id_or_fields, nom, poids, prix, paiement_id, vrac, panier_statut_id){
	...
	$.ajax({
		// Server script
		url: rel+'c/php/admin/admin_ajax.php?create_panier&nom='+nom+'&poids='+poids+'&prix='+prix+'&paiement_id='+paiement_id+'&statut_id='+panier_statut_id,
		type: 'GET',
				...
				// article_id_or_fields is article id
					...
					if(vrac*1 !== 1){
						...
						if( $('div#formsContainer').length ){
							...
							$('div#adminContainer').append('<a class="button articles edit artSH" href="'+rel+'c/admin/articles.php">Articles</a> <a class="button vente edit venSH" href="'+rel+'c/admin/ventes.php">Ventes</a><br>'+message);
						}
					}
...
$(document).ready(function(){
	...
	*/
	...
	$('div#paniersAjaxTarget, div#ventesPaniersAjaxTarget').on('click', 'a.deletePanier', function(){
		...
		$.ajax({
			// Server script 
			url: rel+'c/php/admin/admin_ajax.php?deleteItem&table='+table+'&id='+paniers_id,
			type: 'GET',
			// on success show message
			success : function(msg) {
				var pre = msg.substr(0, 2);
		...
		// just update article to sold with existing paniers_id
			...
			// if not vrac, just update article and panier
				...
				if( $('div#formsContainer').length ){
					//alert('yes formsContainer!');
					$('div#formsContainer').hide();
					$('div#adminContainer').append('<a class="button articles edit artSH" href="'+rel+'c/admin/articles.php">Articles</a> <a class="button vente edit venSH" href="'+rel+'c/admin/ventes.php">Ventes</a><br>');
				}
	...
	$("select[name='categories_id'], select[name='matieres_id']").on('change', function(){
		...
		// call to c/js/js.js: function get_children(), that will request via ajax call to c/php/admin/admin_ajax.php?get_children the children of id_parent in table, and insert them as html <option> markup to $target
	...
	// AND initiates ajax call to upload via c/php/admin/admin_ajax.php -> upload_file()
